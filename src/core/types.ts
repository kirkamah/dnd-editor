...
/**
 * v1.4: табличка с именем персонажа — рисуется выше рамки портретов.
 * Два вида: текстовая (text + стиль прямоугольника) и картинкой (image,
 * старый формат). Если задан image — рисуется картинка, иначе — текст.
 */
export interface PlateEntry {
  /** картинка-табличка (art/...) — старый формат */
  image?: string;
  /** текст таблички — новый формат (прямоугольник с надписью) */
  text?: string;
  /** цвет фона прямоугольника (текстовая табличка) */
  bg?: string;
  /** цвет текста */
  color?: string;
  /** скругление углов прямоугольника, px */
  radius?: number;
  /** кегль текста, px сцены */
  fontSize?: number;
  x: number;
  y: number;
  w: number;
